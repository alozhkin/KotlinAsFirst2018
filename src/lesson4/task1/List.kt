 ...
 */
// F -- Feminine or Female, M -- Masculine or Man, N -- Neuter or No
enum class Gender {
    F, M, N
}
//записывает прописью по-русски натуральное число, учитывая его род (не больше трёх разрядов)
fun threeDigNumToRus(n: Int, gen: Gender): String {
    if (n !in 0..999) {
        throw IllegalArgumentException("n может принимать значения от 1 до 999")
    }

    val result = mutableListOf<String>()
    val firstDigit = n % 10
    val secondDigit = n / 10 % 10
    val thirdDigit = n / 100
    ...
    if (n % 100 in 10..19) {
        ...
        result.add(if (firstDigit in 1..2) {
            if (gen == Gender.M) {
                if (firstDigit == 1) "один" else "два"
            } else if (gen == Gender.F) {
                if (firstDigit == 1) "одна" else "две"
            } else if (firstDigit == 1) "одно" else "два"
        } else {
            when (firstDigit) {
                3 -> "три"
                4 -> "четыре"
